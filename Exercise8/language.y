...
@attributes { table *var_table; tree_node *node; int num_params; } FuncCall
...
Term: '(' Expr ')'
    ...
    | IDENTIFIER '(' FuncCall ')'
        @{
            @i @Term.node@ = new_func(@IDENTIFIER.name@, @FuncCall.num_params@, @FuncCall.node@);
...
FuncCall: Expr ',' FuncCall
        @{
            ...
            @i @FuncCall.0.num_params@ = 1 + @FuncCall.1.num_params@;

            @i @FuncCall.0.node@ = new_param(@Expr.node@, @FuncCall.1.node@);
            @reggen calc_register(@FuncCall.0.node@);
        @}
    | Expr
        @{
            @i @Expr.var_table@ = @FuncCall.var_table@;
            @i @FuncCall.0.node@ = new_param(@Expr.node@, new_empty());
            @i @FuncCall.0.num_params@ = 1;
    ...
    | /* empty */
        @{
            @i @FuncCall.0.node@ = new_empty();
            @i @FuncCall.0.num_params@ = 0;
        @}
